...
#pragma warning restore 0414
        ...
        private void Start()
        {
            XRHandSubsystem m_Subsystem = XRGeneralSettings.Instance?
                                                           .Manager?
                                                           .activeLoader?
        ...
        private void OnEnable()
        {
            ...
            if(LynxHandtrackingAPI.HandSubsystem != null)
            {
        ...
        private void OnDisable()
        {
            if (LynxHandtrackingAPI.HandSubsystem != null)
            {
                ...
                if (leftLine) DestroyHandsLine(left);
                if (rightLine) DestroyHandsLine(right);
                if (leftMarker) DestroyHandsMarker(left);
                if (rightMarker) DestroyHandsMarker(right);
            }
        ...
        private void GenerateHandMarker(XRHand hand)
        {
            ...
            for (var i = XRHandJointID.BeginMarker.ToIndex(); i < XRHandJointID.EndMarker.ToIndex(); i++)
            {
                ...
                markers[i].SetActive(false);//Because rare bug where scale 1 sphere primitive remaining at origin 0
            }
        ...
        /// <summary>
        /// Update transforms of the tracked hands marker root parent then all the joint markers
        /// </summary>
        /// <param name="subsystem"></param>
        /// <param name="updateSuccessFlags"></param>
        /// <param name="updateType"></param>
        private void OnHandUpdateMarker(XRHandSubsystem subsystem, XRHandSubsystem.UpdateSuccessFlags updateSuccessFlags, XRHandSubsystem.UpdateType updateType)
        {
        ...
        private void UpdateHandMarkerTrs(XRHand hand, GameObject[] markers)
        {
            ...
            for (var i = XRHandJointID.BeginMarker.ToIndex(); i < XRHandJointID.EndMarker.ToIndex(); i++)
            {
                ...
                if (trackingData.TryGetPose(out Pose pose))
                {
                    ...
                    if(markers[i].activeSelf == false) markers[i].SetActive(true);
                }
            }
        }
        /// <summary>
